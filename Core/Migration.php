...
abstract class Migration
{
     ...
     */
    protected function addForeignKey($tableName, $columnName, $referencedTable, $referencedColumn)
    {
        return $this->execute("ALTER TABLE `{$tableName}` ADD CONSTRAINT `{$tableName}_{$columnName}_fk` FOREIGN KEY (`{$columnName}`) REFERENCES `{$referencedTable}` (`{$referencedColumn}`);"); 
    }
